...
from panel_code_ode.core.source_doublet.doublet_functions import compute_vortex_line_ind_vel

def free_wake_comp(num_nodes, mesh_dict, mu, sigma, wake_mesh_dict, mu_wake, vc=1.e-6):
    ...
    for i in range(len(surface_names)):
        ...
        eval_pt_i = wake_mesh_dict[surf_name_i]['mesh'][:,:,:,:] # evaluating at the mesh nodes
        ...
        for j in range(len(surface_names)):
            ...
            panel_corners_s_j = mesh_dict[surf_name_j]['panel_corners'][:,:,:,:,:]
            panel_x_dir_s_j = mesh_dict[surf_name_j]['panel_x_dir'][:,:,:,:]
            panel_y_dir_s_j = mesh_dict[surf_name_j]['panel_y_dir'][:,:,:,:]
            panel_normal_s_j = mesh_dict[surf_name_j]['panel_normal'][:,:,:,:]
            dpij_s_j = mesh_dict[surf_name_j]['dpij'][:,:,:,:,:]
            dij_s_j = mesh_dict[surf_name_j]['dij'][:,:,:,:]
            mij_s_j = mesh_dict[surf_name_j]['mij'][:,:,:,:]
            ...
            ind_vel_s_12 = compute_vortex_line_ind_vel(panel_corners_s_j_exp_vec[:,:,0,:],panel_corners_s_j_exp_vec[:,:,1,:],p_eval=eval_pt_w_i_exp_vec[:,:,0,:], mode='wake', vc=vc)
            ind_vel_s_23 = compute_vortex_line_ind_vel(panel_corners_s_j_exp_vec[:,:,1,:],panel_corners_s_j_exp_vec[:,:,2,:],p_eval=eval_pt_w_i_exp_vec[:,:,0,:], mode='wake', vc=vc)
            ind_vel_s_34 = compute_vortex_line_ind_vel(panel_corners_s_j_exp_vec[:,:,2,:],panel_corners_s_j_exp_vec[:,:,3,:],p_eval=eval_pt_w_i_exp_vec[:,:,0,:], mode='wake', vc=vc)
            ind_vel_s_41 = compute_vortex_line_ind_vel(panel_corners_s_j_exp_vec[:,:,3,:],panel_corners_s_j_exp_vec[:,:,0,:],p_eval=eval_pt_w_i_exp_vec[:,:,0,:], mode='wake', vc=vc)
            ...
            panel_corners_w_j = wake_mesh_dict[surf_name_j]['panel_corners'][:,:,:,:,:]
            ...
            ind_vel_w_12 = compute_vortex_line_ind_vel(panel_corners_w_j_exp_vec[:,:,0,:],panel_corners_w_j_exp_vec[:,:,1,:],p_eval=eval_pt_w_i_exp_vec, mode='wake', vc=vc)
            ind_vel_w_23 = compute_vortex_line_ind_vel(panel_corners_w_j_exp_vec[:,:,1,:],panel_corners_w_j_exp_vec[:,:,2,:],p_eval=eval_pt_w_i_exp_vec, mode='wake', vc=vc)
            ind_vel_w_34 = compute_vortex_line_ind_vel(panel_corners_w_j_exp_vec[:,:,2,:],panel_corners_w_j_exp_vec[:,:,3,:],p_eval=eval_pt_w_i_exp_vec, mode='wake', vc=vc)
            ind_vel_w_41 = compute_vortex_line_ind_vel(panel_corners_w_j_exp_vec[:,:,3,:],panel_corners_w_j_exp_vec[:,:,0,:],p_eval=eval_pt_w_i_exp_vec, mode='wake', vc=vc)
    ...
    for nn in csdl.frange(num_nodes):
        for direction in csdl.frange(3):
            ...
            mu_surf_induced_vel = csdl.matvec(AIC_mu_surf[nn,:,:,direction], mu[nn,:])
            mu_wake_induced_vel = csdl.matvec(AIC_mu_wake[nn,:,:,direction], mu_wake[nn,:])
